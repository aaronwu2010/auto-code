...
const (
	// NoToolsPreamble 无工具前缀
	// 在缓存共享fork路径中，子进程继承父进程的完整工具集
	// 在Sonnet 4.6+自适应思考模型上，模型有时会尝试工具调用
	// 即使有较弱的后缀指令。使用maxTurns: 1时，拒绝的工具调用
	// 意味着没有文本输出 → 回退到流式fallback (4.6上2.79% vs 4.5上0.01%)
	// 将此放在第一位并明确拒绝后果可以防止浪费的turn
...
`
	...
	// NoToolsTrailer 无工具后缀
	NoToolsTrailer = "\n\nREMINDER: Do NOT call any tools. Respond with plain text only — an <analysis> block followed by a <summary> block. Tool calls will be rejected and you will fail the task."
)

// DetailedAnalysisInstructionBase 详细分析指令（基础版本）
...
// DetailedAnalysisInstructionPartial 详细分析指令（部分版本）
...
// PartialCompactUpToPrompt 部分压缩（向上）提示词
...
func GetCompactPrompt(customInstructions string) string {
	prompt := NoToolsPreamble + fmt.Sprintf(BaseCompactPrompt, DetailedAnalysisInstructionBase)
...
func GetPartialCompactPrompt(customInstructions string, direction CompactDirection) string {
	...
	prompt := NoToolsPreamble + fmt.Sprintf(template, DetailedAnalysisInstructionPartial)
...
// 通过去除<analysis>草稿区域并替换<summary> XML标签为可读的章节标题
func FormatCompactSummary(summary string) string {
	formattedSummary := summary

	// 去除分析部分 - 它是一个草稿区域，可以提高摘要质量
	// 但在摘要写完后没有信息价值
	formattedSummary = strings.Replace(formattedSummary, "<analysis>", "", -1)
	formattedSummary = strings.Replace(formattedSummary, "</analysis>", "", -1)
	...
	// 清理章节间的多余空白
	formattedSummary = strings.ReplaceAll(formattedSummary, "\n\n\n+", "\n\n")
